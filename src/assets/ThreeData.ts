...
import { ThreeDataList } from '../declarations/types.td';
...
const THREE_DATA: ThreeDataList = [...Array(images.length).keys()].map((i) => {
  return {
    key: String(i),
...
export const THREE_AVATARS: string[] = [...Array(6).keys()].map(faker.internet.avatar);

export default THREE_DATA;
